...
const showGrid = (data, img_urls_small, img_urls_big) => {
	...
	img_urls_small.forEach((url) => {
		i++;
		html += `<button class="grid-artist o-button-reset" data-nr="${i}" data-name="${data[i - 1].track.artists[0].name}" data-id="${
			data[i - 1].track.artists[0].id
		}">
...
const showModal = (data, img_urls, id, nr, name) => {
	...
	document.querySelector(".artist-song-img").innerHTML = `<img src="${data[nr - 1].track.album.images[1].url}" alt="Album cover for song: ${
		data[nr - 1].track.name
	}">`;
